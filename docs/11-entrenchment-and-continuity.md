...
Research on transactive memory describes how groups develop shared knowledge about who knows what; performance can depend not only on individual memory but on a system for locating expertise.[^1] Organizational knowledge can also move through people, tools, routines, and networks rather than through documents alone.[^2] The practical implication is that continuity requires more than asking an expert to write everything down.
